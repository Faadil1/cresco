/**
 * CRESCO backend adapter (repository root: src/http-api.mjs).
 *
 * Configuration (build-time public env; no secrets ever live here):
 *   NEXT_PUBLIC_CRESCO_API_URL=http://127.0.0.1:8787   optional CRESCO API override
 *   NEXT_PUBLIC_CRESCO_EXECUTION=runtime                 optional execution override
 *
 * Production fallback:
...
type CrescoConfig = { url: string; execution: "demo" | "runtime"; timeoutMs: number };

const PUBLIC_HOSTED_CRESCO_API =
  "https://keys-api-stocklana.faadil-casecraft.workers.dev";

let override: Partial<CrescoConfig> | null = null;
...
const BACKEND_ACTIVE_ROLE_KEY = "cresco-session-active-role-v1";
...
export function configureCrescoBackend(next: Partial<CrescoConfig> | null) {
  override = next;
}
...
export function crescoConfig(): KeysConfig {
  ...
  const url = (
    override?.url ??
    process.env.NEXT_PUBLIC_CRESCO_API_URL ??
    productionFallback
  ).replace(/\/$/, "");

  const envExecution = process.env.NEXT_PUBLIC_CRESCO_EXECUTION;
  const execution =
    override?.execution ??
    (envExecution
...
export function crescoBackendConfigured() {
  return crescoConfig().url.length > 0;
}

export function crescoRuntimeExecutionEnabled() {
  return crescoConfig().execution === "runtime";
}

/** Base URL for display (technical details). */
export function crescoApiUrl() {
  return crescoConfig().url;
}
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const res = await fetch(`${crescoConfig().url}${path}`, {
    ...init,
    headers: {
...
export type CrescoCapabilities = {
...
export function fetchCapabilities() {
  return request<CrescoCapabilities>("/api/v0.1/capabilities");
}
...
export async function evaluateAction(input: {
  ...
  return { ...result, source: "cresco-backend" };
}
...
function parseExecuteResponse(raw: unknown, idempotencyKey: string): ExecuteResult | null {
  ...
  const evaluation: ActionEvaluation = { ...e, source: "cresco-runtime" };
...
export async function executeAction(body: ExecuteRequest): Promise<ExecuteResult> {
  let res: Response;
  try {
    res = await fetch(`${crescoConfig().url}/api/v0.2/actions/execute`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
  ...
  if (!res.ok) {
    return {
      outcome: "REFUSED",
      evaluation: { decision: "REFUSE", reasonCode: res.status === 409 ? "STALE_NONCE" : "DECISION_UNAVAILABLE", source: "cresco-runtime" },
    };
  }
